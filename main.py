...
async def upload(file: UploadFile = File(...)):
    try:
        ...
        # OPTIONAL Email sheet log (Sheet2)
        try:
            subject = f"Compliance Report – {file.filename}"
            body = f"""
File: {file.filename}
Risk Score: {analysis.get('risk_score')}
Missing Clauses: {', '.join(analysis.get('missing_clauses', []))}
"""
            write_email_log("compliance-team@company.com", subject, body)
...
# -----------------------------------------------------
@app.get("/sheet_history")
def sheet_history():
    try:
        rows = read_history_rows()
        return JSONResponse(content=rows)
    except Exception as e:
        logger.exception("sheet_history error")
        raise HTTPException(status_code=500, detail=str(e))
